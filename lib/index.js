...
/**
 * Normalized value
 * @typedef {{ normalizedValue: number, rawValue: number, formattedValue: string, color: string, label: string, lineLength: number, chartBar: string, coloredChartBar: string }} NormalizedValue
 */
...
function draw(values = [], options = {}) {
  ...
  const chart = normalizedValues
    ...
    .map(({ coloredChartBar }) => coloredChartBar);

  const legend = normalizedValues.map(value => {
    const coloredBlock =
      typeof chalk[value.color] === 'function'
        ? chalk[value.color](SYMBOLS.Full)
        : chalk.hex(value.color)(SYMBOLS.Full);
  ...
  let scale = [
    [SYMBOLS.LeftCorner, minValueFormatted].join(SYMBOLS.Space).trim(),
    [maxValueFormatted, SYMBOLS.RightCorner].join(SYMBOLS.Space).trim(),
  ];
