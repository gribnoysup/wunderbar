...
const normalizeValues = values => {
  if (!Array.isArray(values)) {
    ...
    throw new TypeError(
      `Unsupported "values" argument: ${stringified}. Expected Array<{ value: number, color?: string, label?: string }>`
    );
  }
  ...
  return values.map(value => {
    if (typeof value === 'object' && typeof value.value !== 'undefined') {
      ...
      throw new TypeError(
        `Unsupported value in "values" argument: ${stringified}. Expected { value: number, color?: string, label?: string }`
      );
    }
  });
};
...
const normalizeOptions = options => {
  ...
  if (typeof options.min !== 'undefined' && isNaN(parseFloat(options.min))) {
    throw new TypeError(
      `Unsupported options.min: ${options.min}. Expected number`
    );
  }

  if (typeof options.max !== 'undefined' && isNaN(parseFloat(options.max))) {
    throw new TypeError(
      `Unsupported options.max: ${options.min}. Expected number`
    );
  }
  ...
  if (
    ...
    throw new TypeError(
      `Unsupported options.length: ${options.length}. Expected number`
    );
  }
 ...
 *
 * @returns {Object<{ legend: string, chart: string, __raw: object }>} output that you can print to the terminal
 */
function draw(values = [], options = {}) {
  ...
  const normalizedValues = values.map((value, index) => {
    ...
    return {
      ...
      lineLength: Math.round(normalizedValue / (maxValue - minValue) * chartLength),
    };
  });
